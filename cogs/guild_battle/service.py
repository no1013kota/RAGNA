...
Discordの操作部品（View）は ``views.py`` とその兄弟モジュール（``*_views.py``）、
Cog本体と定期タスクは ``cog.py`` に置き、このモジュールは「開始前チェック」
「バトル開始」「行動の適用」「終了処理と報酬」といった、複数の入口から共通で
使う処理だけを持ちます。

Viewを使う箇所（``_command_view``）は関数内の遅延importのままにしてください。
先頭のimportへ移すと ``views`` との循環importになります。
